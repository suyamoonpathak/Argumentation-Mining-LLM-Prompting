...
def get_three_way_prediction(text, filename):
    ...
    try:
        ...
        prompt = f"""You are a Harvard-trained legal expert specialized in legal argumentation analysis, with deep expertise in distinguishing "premise", "conclusion", and "non-argumentative" text in legal case files.
            ...
            PREMISE Examples:
            1. "Moreover, contrary to what the Hellenic Republic claims, the classification as State aid does not apply to the contributions paid by farmers but to the payments made in 2008."
            [Analysis: Provides supporting reasoning with "Moreover" and establishes factual distinction]

            2. "In that regard, it must be recalled that it has been held in the context of the first and second pleas in law that neither the compensatory nature of the measures, nor the fact that they are justified by a social objective, precludes the payments made by ELGA being classified as an advantage."
            [Analysis: Establishes legal principle as foundation for argument]
            ...
            2. "In those circumstances, the second part of this ground of appeal must be rejected as, in part, unfounded and, in part, inadmissible."
            [Analysis: Ultimate ruling based on preceding circumstances]

            NON-ARGUMENTATIVE Examples:
            1. "That revenue shall be paid to ELGA from the budget of the Ministry of Agriculture by a transfer of funds in the same amount each year following a proposal to that Ministry by ELGA."
            [Analysis: Pure procedural description without interpretation]

            2. "The State shall withhold two percent (2%) of the special insurance contribution amounts collected by the public finance services for ELGA for the collection thereof."
            [Analysis: Factual statement of legal requirement without argument]
        ...
        genai.configure(api_key=api_keys[current_key_index])
        return get_three_way_prediction(text, filename)
