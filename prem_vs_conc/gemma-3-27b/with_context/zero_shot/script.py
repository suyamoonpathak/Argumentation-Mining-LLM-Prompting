...
def get_premise_conclusion_prediction(text, filename):
    ...
    try:
        ...
        prompt = f""" You are a Harvard-trained legal expert specialized in legal argumentation analysis, with deep expertise in distinguishing "premise" from "conclusion" text in legal case files.

            ## TASK: Your task is to read the given legal sentence and decide if it is a "premise" or a "conclusion" in a legal argument. Your analysis is very important. Your answer will be used in a legal research tool for senior judges working on high-impact cases. A wrong answer could cause legal mistakes, unfair judgments, or even wrongful convictions. So be very careful and precise.

            ## DEFINITIONS:
            - PREMISE: A sentence that gives reasons, evidence, facts, or legal rules to support another statement. It is part of the reasoning but does not finish it.
            - CONCLUSION: A sentence that gives the final decision, main point, or end result of the reasoning.
        ...
        genai.configure(api_key=api_keys[current_key_index])
        return get_premise_conclusion_prediction(text, filename)
